...
public class SoundTriangle extends Sound2A03 {
	...
	private int smoothSteps = 1;

	public void setSmoothSteps(int steps) {
		this.smoothSteps = Math.max(1, steps);
	}
	...
	protected void onProcess(int time) {
		...
		while (time >= counter) {
			...
			if (m <= 1) {
				...
				int next = TRIANGLE_WAVE[dutyCycle];
				int delta = next - value;
				for (int k = 0; k < m; k++) {
					int v = value * m + delta * k;
					int off = (int) ((long) counter * k / m);
					mix(v, off);
				}
			}
		}
