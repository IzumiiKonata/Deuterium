...
public class Xgm2A07Mixer extends AbstractXgmMultiMixer {
	...
	private int triDivisor = 1;

	private static final double TND_VMAX = 15.0 / 8227 + 15.0 / 12241 + 127.0 / 22638;
	private static final double TND_LINEAR = (981750.0 / (100.0 + 1.0 / TND_VMAX)) / TND_VMAX;

	public void setLinearMixing(boolean linearMixing) {
		this.linearMixing = linearMixing;
	}

	public void setTriDivisor(int triDivisor) {
		this.triDivisor = Math.max(1, triDivisor);
	}
	...
	public int render(int index) {
		 ...
		 */
		float v0 = (triEnable) ? tri.read(index) * tri.getLevel() / 8227 / triDivisor : 0;
		float v1 = (noiseEnable) ? noise.read(index) * noise.getLevel() / 12241 : 0;
		float v2 = (dpcmEnable) ? dpcm.read(index) * dpcm.getLevel() / 22638 : 0;
		float v = v0 + v1 + v2;
		int value;
		if (linearMixing) {
			value = (int) (TND_LINEAR * v);
		} else {
			value = (v != 0) ? (int) ((MASTER) / (100.0 + 1.0 / v)) : 0;
		}
