...
import tritium.zdream.nsfplayer.core.NsfCommonParameter;
import tritium.zdream.nsfplayer.core.NsfPlayerException;
import tritium.zdream.nsfplayer.mixer.AbstractNsfSoundMixer;
import tritium.zdream.nsfplayer.mixer.IMixerHandler;
import tritium.zdream.nsfplayer.mixer.ITrackMixer;
import tritium.zdream.nsfplayer.mixer.interceptor.Amplifier;
import tritium.zdream.nsfplayer.mixer.interceptor.Compressor;
import tritium.zdream.nsfplayer.mixer.interceptor.DCFilter;
import tritium.zdream.nsfplayer.mixer.interceptor.EchoUnit;
import tritium.zdream.nsfplayer.mixer.interceptor.Filter;
import tritium.zdream.nsfplayer.mixer.interceptor.ISoundInterceptor;
...
public class XgmMultiSoundMixer extends AbstractNsfSoundMixer<AbstractXgmAudioChannel>
	...
	private boolean linearMixing = false;
	private int triSmooth = 1;

	public void setLinearMixing(boolean linear) {
		this.linearMixing = linear;
		for (AbstractXgmMultiMixer multi : multiList) {
			if (multi instanceof Xgm2A03Mixer) {
				((Xgm2A03Mixer) multi).setLinearMixing(linear);
			} else if (multi instanceof Xgm2A07Mixer) {
				((Xgm2A07Mixer) multi).setLinearMixing(linear);
			}
		}
	}
...
//		}
	...
	protected XgmMultiChannelAttr createChannelAttr(final byte code) {
		...
		for (AbstractXgmMultiMixer multi : multiList) {
			channel = multi.getRemainAudioChannel(type);
			if (channel != null) {
				// 将该轨道插入到原来已经存在的合并轨道中
	...
	private AbstractXgmMultiMixer createMultiChannelMixer(byte chip) {
		...
		if (multi != null) {
			...
			if (multi instanceof Xgm2A03Mixer) {
				((Xgm2A03Mixer) multi).setLinearMixing(linearMixing);
			} else if (multi instanceof Xgm2A07Mixer) {
	...
	public void readyBuffer() {
		...
		attrs.stream()
				...
				.forEach(attr -> readyChannelInRuntime((XgmMultiChannelAttr) attr));
	}

	@Override
	public void readyChannelInRuntime(final byte code) {
		attrs.stream()
				.filter(Objects::nonNull)
				.filter(attr -> attr.code == code)
				.findFirst()
				.ifPresent(attr -> readyChannelInRuntime((XgmMultiChannelAttr) attr));
	}

	public void readyChannelInRuntime(XgmMultiChannelAttr a) {
		int inSample = a.inSample;
		if (inSample == 0) {
