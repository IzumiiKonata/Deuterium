...
import tritium.zdream.nsfplayer.mixer.NsfMixerSoundConvertor;
...
public class XgmSingerSoundMixer extends AbstractNsfSoundMixer<XgmSingleChannel>
	...
	protected ChannelAttr createChannelAttr(byte code) {
		...
		c.exp = NsfMixerSoundConvertor.getExpression(typeOfChannel(code));
	...
	public void readyBuffer() {
		...
		attrs.stream()
				...
				.forEach(this::readyChannelInRuntime);
	}

	@Override
	public void readyChannelInRuntime(byte code) {
		attrs.stream()
				.filter(Objects::nonNull)
				.filter(attr -> attr.code == code)
				.findFirst()
				.ifPresent(this::readyChannelInRuntime);
	}

	private void readyChannelInRuntime(ChannelAttr attr) {
