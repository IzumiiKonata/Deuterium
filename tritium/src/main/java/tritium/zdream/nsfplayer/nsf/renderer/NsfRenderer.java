package tritium.zdream.nsfplayer.nsf.renderer;

import java.util.Set;

import tritium.zdream.nsfplayer.core.AbstractNsfRenderer;
import tritium.zdream.nsfplayer.core.CycleCounter;
import tritium.zdream.nsfplayer.core.FloatCycleCounter;
...
import tritium.zdream.nsfplayer.nsf.executor.NsfExecutor;
...
public class NsfRenderer extends AbstractNsfRenderer<NsfAudio> {
	...
	public AbstractNsfSound getSound(byte channelCode) {
		return executor.getSound(channelCode);
	}

	public int getClockRate() {
		return executor.cycleRate();
	}
			...
			for (int i = 0; i < 8; i++) {
				...
				if (sound != null) {
					...
					if (p == null) {
						...
						sound.setOut(mix);
						mix.setLevel(getInitLevel(channelCode));
						mixer.readyChannelInRuntime(channelCode);
