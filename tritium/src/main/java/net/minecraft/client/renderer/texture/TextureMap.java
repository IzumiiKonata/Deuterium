...
public class TextureMap extends AbstractTexture implements ITickableTextureObject {
    ...
    private static final Logger logger = LogManager.getLogger("TextureMap");
    public static final Location LOCATION_MISSING_TEXTURE = Location.of("missingno");
    public static final Location locationBlocksTexture = Location.of("textures/atlas/blocks.png");
        ...
        while (true) {
            if (iterator.hasNext()) {
                ...
                if (!this.skipFirst) {
                    ...
                    Location resourcelocation1 = Location.of(textureatlassprite3.getIconName());
                    Location resourcelocation2 = this.completeResourceLocation(resourcelocation1, 0);
...
//                        }
            ...
            for (TextureAtlasSprite textureatlassprite2 : stitcher.getStichSlots()) {
                ...
                try {
                    ...
                    CrashReport crashreport1 = CrashReport.makeCrashReport(throwable, "拼接纹理图集");
    ...
    public Location completeResourceLocation(Location p_completeResourceLocation_1_) {
        return this.completeResourceLocation(p_completeResourceLocation_1_, 0);
    }

    public Location completeResourceLocation(Location location, int p_147634_2_) {
        return this.isAbsoluteLocation(location) ? Location.of(location.getResourceDomain(), location.getResourcePath() + ".png") : (p_147634_2_ == 0 ? Location.of(location.getResourceDomain(), String.format("%s/%s%s", this.basePath, location.getResourcePath(), ".png")) : Location.of(location.getResourceDomain(), String.format("%s/mipmaps/%s.%d%s", this.basePath, location.getResourcePath(), p_147634_2_, ".png")));
    }
    ...
    public TextureAtlasSprite registerSprite(Location location) {
        if (location == null) {
    ...
    private boolean isAbsoluteLocation(Location p_isAbsoluteLocation_1_) {
    ...
    public TextureAtlasSprite getSpriteSafe(String p_getSpriteSafe_1_) {
        Location resourcelocation = Location.of(p_getSpriteSafe_1_);
        return this.mapRegisteredSprites.get(resourcelocation.toString());
    }

    public TextureAtlasSprite getRegisteredSprite(Location p_getRegisteredSprite_1_) {
    ...
    private int detectMinimumSpriteSize(Map p_detectMinimumSpriteSize_1_, IResourceManager p_detectMinimumSpriteSize_2_, int p_detectMinimumSpriteSize_3_) {
        ...
        for (Object o : p_detectMinimumSpriteSize_1_.entrySet()) {
            ...
            Location resourcelocation = Location.of(textureatlassprite.getIconName());
            Location resourcelocation1 = this.completeResourceLocation(resourcelocation);
    ...
    private void checkEmissive(Location p_checkEmissive_1_, TextureAtlasSprite p_checkEmissive_2_) {
        String s = EmissiveTextures.getSuffixEmissive();

        if (s != null) {
            if (!p_checkEmissive_1_.getResourcePath().endsWith(s)) {
                Location resourcelocation = Location.of(p_checkEmissive_1_.getResourceDomain(), p_checkEmissive_1_.getResourcePath() + s);
                Location resourcelocation1 = this.completeResourceLocation(resourcelocation);
