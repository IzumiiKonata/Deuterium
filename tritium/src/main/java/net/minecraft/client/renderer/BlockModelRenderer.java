...
public class BlockModelRenderer {
    ...
    public boolean renderModel(IBlockAccess blockAccessIn, IBakedModel modelIn, IBlockState blockStateIn, BlockPos blockPosIn, WorldRenderer worldRendererIn, boolean checkSides) {
        ...
        try {
            ...
            CrashReport crashreport = CrashReport.makeCrashReport(throwable, "曲面细分方块模型");
            CrashReportCategory crashreportcategory = crashreport.makeCategory("正在进行细分曲面的方块");
    ...
    public static class AmbientOcclusionFace {
        ...
        public void updateVertexBrightness(IBlockAccess blockAccessIn, Block blockIn, BlockPos blockPosIn, EnumFacing facingIn, float[] quadBounds, BitSet boundsFlags) {
            ...
            // Fix MC-43968 by using the correct positions for determining whether neighboring blocks are opaque

            boolean flag = blockAccessIn.getBlockState(blockpos1/*.offset(facingIn)*/).getBlock().isTranslucentAO();
            boolean flag1 = blockAccessIn.getBlockState(blockpos2/*.offset(facingIn)*/).getBlock().isTranslucentAO();
            boolean flag2 = blockAccessIn.getBlockState(blockpos3/*.offset(facingIn)*/).getBlock().isTranslucentAO();
            boolean flag3 = blockAccessIn.getBlockState(blockpos4/*.offset(facingIn)*/).getBlock().isTranslucentAO();
            float f4;
            int i1;
