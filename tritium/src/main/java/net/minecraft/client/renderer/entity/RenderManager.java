...
public class RenderManager {
        ...
        try {
            ...
            if (render != null && this.renderEngine != null) {
                try {
                    ...
                    throw new ReportedException(CrashReport.makeCrashReport(throwable2, "渲染世界中的实体"));
                }
                ...
                try {
                    ...
                    throw new ReportedException(CrashReport.makeCrashReport(throwable1, "后渲染实体效果"));
                }
                ...
                if (this.debugBoundingBox && !entity.isInvisible() && !hideDebugBox) {
                    try {
                        ...
                        throw new ReportedException(CrashReport.makeCrashReport(throwable, "渲染世界中实体的碰撞箱"));
                    }
                }
            ...
            CrashReport crashreport = CrashReport.makeCrashReport(throwable3, "渲染世界中的实体");
            CrashReportCategory crashreportcategory = crashreport.makeCategory("被渲染的实体");
            entity.addEntityCrashInfo(crashreportcategory);
            CrashReportCategory crashreportcategory1 = crashreport.makeCategory("渲染器信息");
            crashreportcategory1.addCrashSection("被分配的渲染器", render);
            crashreportcategory1.addCrashSection("位置", CrashReportCategory.getCoordinateInfo(x, y, z));
            crashreportcategory1.addCrashSection("旋转角度", entityYaw);
            crashreportcategory1.addCrashSection("帧增量", partialTicks);
    ...
    private void renderDebugBoundingBox(Entity entityIn, double x, double y, double z, float entityYaw, float partialTicks) {
        if (!Shaders.isShadowPass) {
            ...
            worldrenderer.pos(x + vec3.xCoord * lineWidth, y + (double) entityIn.getEyeHeight() + vec3.yCoord * lineWidth, z + vec3.zCoord * lineWidth).color(0, 0, 255, 255).endVertex();
            tessellator.draw();
