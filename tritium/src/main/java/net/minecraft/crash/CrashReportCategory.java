...
public class CrashReportCategory {
    ...
    public static String getCoordinateInfo(BlockPos pos) {
        ...
        try {
            stringbuilder.append(String.format("世界: (%d,%d,%d)", i, j, k));
        } catch (Throwable var17) {
            stringbuilder.append("(寻找世界位置时出现错误)");
        }
        ...
        try {
            ...
            int k2 = (l + 1 << 4) - 1;
            int l2 = (i1 + 1 << 4) - 1;
            stringbuilder.append(String.format("区块: (在 %d,%d,%d in %d,%d; 包含方块 从 %d,0,%d 到 %d,255,%d)", j1, k1, l1, l, i1, i2, j2, k2, l2));
        } catch (Throwable var16) {
            stringbuilder.append("(寻找区块位置时出现错误)");
        }
        ...
        try {
            ...
            int j5 = (j3 + 1 << 9) - 1;
            int i3 = (k3 + 1 << 9) - 1;
            stringbuilder.append(String.format("区域: (%d,%d; 包含区块 从 %d,%d 到 %d,%d, 方块 从 %d,0,%d 到 %d,255,%d)", j3, k3, l3, i4, j4, k4, l4, i5, j5, i3));
    ...
    public static void addBlockInfo(CrashReportCategory category, final BlockPos pos, final Block blockIn, final int blockData) {
        ...
        category.addCrashSectionCallable("方块类型", () -> {
            try {
