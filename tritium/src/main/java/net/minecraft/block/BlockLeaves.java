...
public abstract class BlockLeaves extends BlockLeavesBase {
    ...
    @Override
    public boolean shouldSideBeRendered(final IBlockAccess iBlockAccess, final BlockPos blockPos, final EnumFacing enumFacing) {

        boolean b = super.shouldSideBeRendered(iBlockAccess, blockPos, enumFacing);

        final byte depth = 8;

        valkyrie$mutableBlockPos.set(blockPos.getX(), blockPos.getY(), blockPos.getZ());

        final World world = Minecraft.getMinecraft().theWorld;

        for (byte i = 1; i <= depth; i++) {
            valkyrie$mutableBlockPos.offset(enumFacing);
