...
public class DataWatcher {
    ...
    public byte getWatchableObjectByte(int id) {
        return ((Number) this.getWatchedObject(id).getObject()).byteValue();
    }
    ...
    public float getWatchableObjectFloat(int id) {
        return ((Number) this.getWatchedObject(id).getObject()).floatValue();
    }
    ...
    private DataWatcher.WatchableObject getWatchedObject(int id) {
        ...
        try {
            ...
            CrashReport crashreport = CrashReport.makeCrashReport(throwable, "获取同步实体数据");
            CrashReportCategory crashreportcategory = crashreport.makeCategory("同步实体数据");
    ...
    public void updateWatchedObjectsFromList(List<DataWatcher.WatchableObject> p_75687_1_) {
        ...
        for (DataWatcher.WatchableObject obj : p_75687_1_) {
            DataWatcher.WatchableObject o = this.watchedObjects.get(obj.getDataValueId());

            if (o != null) {

                if (this.owner == Minecraft.getMinecraft().thePlayer && obj.getDataValueId() == 17) {
                    if (((float) obj.getObject()) <= 0.0f) {
                        Minecraft.getMinecraft().thePlayer.maxAbsp = 0.0f;
                    } else {
                        Minecraft.getMinecraft().thePlayer.maxAbsp = Math.max(((float) obj.getObject()), Minecraft.getMinecraft().thePlayer.maxAbsp);
                    }
                }

                o.setObject(obj.getObject());
                this.owner.onDataWatcherUpdate(obj.getDataValueId());
            }
        }
