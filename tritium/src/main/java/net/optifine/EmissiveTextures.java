...
public class EmissiveTextures {
    ...
    private static final Location LOCATION_EMPTY = Location.of("mcpatcher/ctm/default/empty.png");
    ...
    public static ITextureObject getEmissiveTexture(final ITextureObject texture, final Map<Location, ITextureObject> mapTextures) {
        if (!render || !(texture instanceof SimpleTexture simpletexture)) {
            return texture;
        } else {
            Location resourcelocation = simpletexture.locationEmissive;
            if (!renderEmissive) {
    ...
    public static boolean isEmissive(final Location loc) {
        return suffixEmissivePng != null && loc.getResourcePath().endsWith(suffixEmissivePng);
    }

    public static void loadTexture(final Location loc, final SimpleTexture tex) {
            ...
            if (suffixEmissivePng != null) {
                ...
                if (s.endsWith(".png")) {
                    if (s.endsWith(suffixEmissivePng)) {
                        ...
                        final Location resourcelocation = Location.of(loc.getResourceDomain(), s1);
