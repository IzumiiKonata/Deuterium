...
public class SmartLeaves {
    ...
    public static boolean isSameLeaves(final IBlockState state1, final IBlockState state2) {
        if (state1 == state2) {
            ...
            return block == block1 && (block instanceof BlockOldLeaf ? state1.getValue(BlockOldLeaf.VARIANT).equals(state2.getValue(BlockOldLeaf.VARIANT))
    ...
    static IBakedModel getModelCull(final String type, final List updatedTypes) {
        ...
        if (modelmanager == null) {
            ...
            final Location resourcelocation = Location.of("blockstates/" + type + "_leaves.json");
            if (Config.getDefiningResourcePack(resourcelocation) != Config.getDefaultResourcePack()) {
                return null;
            } else {
                final Location resourcelocation1 = Location.of("models/block/" + type + "_leaves.json");
    ...
    private static IBakedModel getModelDoubleFace(final IBakedModel model) {
        if (model == null) {
            ...
            for (final EnumFacing enumfacing : aenumfacing) {
