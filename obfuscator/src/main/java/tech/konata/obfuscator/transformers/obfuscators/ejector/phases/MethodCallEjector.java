...
package tech.konata.obfuscator.transformers.obfuscators.ejector.phases;
...
import tech.konata.obfuscator.transformers.obfuscators.ejector.EjectorContext;
...
public final class MethodCallEjector extends AbstractEjectPhase {
    ...
    private static MethodNode createProxyMethod(String name, MethodCallInfo methodCallInfo) {
        ...
        for (int i = 0; i < arguments.size() - 1; i++) {
            ...
            insnList.add(new VarInsnNode(BytecodeUtils.getVarOpcode(type, false), variable));
        ...
        insnList.add(new InsnNode(BytecodeUtils.getReturnOpcode(returnType)));
        ...
        for (int k = 0; k < getJunkArgumentCount(); k++) {
            ...
            for (Type argumentType : argumentTypes) {
                if (RandomUtils.getRandomBoolean()) {
                    junkProxyArgumentFix.add(BytecodeUtils.getRandomValue(argumentType));
                    junkProxyArgumentFix.add(new VarInsnNode(BytecodeUtils.getVarOpcode(argumentType, true), offset + junkVariable));
                }
    ...
    public void process(MethodWrapper methodWrapper, Frame<AbstractValue>[] frames) {
        ...
        methodCalls.forEach((key, value) -> {
            ...
            for (MethodInsnNode methodInsnNode : value) {
                ...
                patches.put(methodInsnNode, BytecodeUtils.asList(
                        new LdcInsnNode(id),
                ...
                for (int i = 0; i < argumentTypes.length; i++) {
                    ...
                    if (argumentValue.isConstant() && argumentValue.getUsages().size() == 1) {
                        AbstractInsnNode valueInsn = ejectorContext.isJunkArguments() ? BytecodeUtils.getRandomValue(argumentType) : BytecodeUtils.getDefaultValue(argumentType);
                        patches.put(argumentValue.getInsnNode(), BytecodeUtils.singletonList(valueInsn));

                        proxyArgumentFix.add(argumentValue.getInsnNode().clone(null));
                        proxyArgumentFix.add(new VarInsnNode(BytecodeUtils.getVarOpcode(argumentType, true), offset + variable));
                    }
