...
public class HeavyStringEncryption extends StringEncryption {
    ...
    private class MemberNames {
        ...
        private MemberNames() {
            ...
            clearSequencedString();
            this.infoFieldName = nextSequencedString();
            this.cacheFieldName = nextSequencedString();
            clearSequencedString();
            this.populateMethodName = nextSequencedString();
            this.createInfoMethodName = nextSequencedString();
            this.setCacheMethodName = nextSequencedString();
            this.getCacheMethodName = nextSequencedString();
            this.cacheContainsMethodName = nextSequencedString();
            this.decryptorMethodName = nextSequencedString();
        }
    }
    ...
    protected boolean excluded(ClassWrapper classWrapper) {
        ...
        boolean clazz = hasAnnotation(classWrapper.classNode.visibleAnnotations, "tech.konata.obfuscation.StringObfuscation");

        if (clazz) {
            String clazzType = getEnumAnnotationValue(classWrapper.classNode.visibleAnnotations, "tech.konata.obfuscation.StringObfuscation", "strength");

            return !clazzType.equals("Heavy");
        } else {

            for (MethodWrapper methodWrapper : classWrapper.methods) {
                boolean method = hasAnnotation(methodWrapper.getMethodNode().visibleAnnotations, "tech.konata.obfuscation.StringObfuscation");

                if (method) {
                    String type = getEnumAnnotationValue(methodWrapper.getMethodNode().visibleAnnotations, "tech.konata.obfuscation.StringObfuscation", "strength");
    ...
    protected boolean excluded(MethodWrapper methodWrapper) {
        ...
        boolean method = hasAnnotation(methodWrapper.getMethodNode().visibleAnnotations, "tech.konata.obfuscation.StringObfuscation");
        boolean clazz = hasAnnotation(methodWrapper.getOwner().classNode.visibleAnnotations, "tech.konata.obfuscation.StringObfuscation");

        if (!(method || clazz))
            return true;

        String type = getEnumAnnotationValue(method ? methodWrapper.getMethodNode().visibleAnnotations : methodWrapper.getOwner().classNode.visibleAnnotations, "tech.konata.obfuscation.StringObfuscation", "strength");
