...
package tech.konata.obfuscator.transformers.obfuscators.invokedynamic;
...
public class NormalInvokeDynamic extends InvokeDynamic {
    ...
    static final int KEY_1 = "IzumiKonata".hashCode();
    static final int KEY_2 = "atanoKimuzI".hashCode();
    static final int KEY_3 = 0x2304;

    @Override
    public void transform() {
        AtomicInteger counter = new AtomicInteger();
        String className = StringUtils.randomClassName(getClasses());
        String bsmName = randomString(4);
        Handle bsmHandle = new Handle(H_INVOKESTATIC, className, bsmName, "(Ljava/lang/Object;" +
                    ...
                    for (AbstractInsnNode insn : methodNode.instructions.toArray()) {
                        if (insn instanceof MethodInsnNode && insn.getOpcode() != INVOKESPECIAL) {
                            ...
                            InvokeDynamicInsnNode indy = new InvokeDynamicInsnNode(StringUtils.randomSpacesString(10),
                                    ...
                                    bsmHandle,
                                    opcode,
                                    encrypt(methodInsnNode.owner.replace("/", "."), KEY_1),
                                    encrypt(methodInsnNode.name, KEY_2),
                                    encrypt(methodInsnNode.desc, KEY_3));
...
//        mv.visitVarInsn(ASTORE, 15);
    ...
    protected boolean excluded(ClassWrapper classWrapper) {
        ...
        boolean clazz = hasAnnotation(classWrapper.classNode.visibleAnnotations, "tech.konata.obfuscation.Indy");

        if (clazz) {
            String clazzType = getEnumAnnotationValue(classWrapper.classNode.visibleAnnotations, "tech.konata.obfuscation.Indy", "strength");

            return !clazzType.equals("Normal");
        } else {

            for (MethodWrapper methodWrapper : classWrapper.methods) {
                boolean method = hasAnnotation(methodWrapper.getMethodNode().visibleAnnotations, "tech.konata.obfuscation.Indy");

                if (method) {
                    String type = getEnumAnnotationValue(methodWrapper.getMethodNode().visibleAnnotations, "tech.konata.obfuscation.Indy", "strength");
    ...
    protected boolean excluded(MethodWrapper methodWrapper) {
        ...
        boolean method = hasAnnotation(methodWrapper.getMethodNode().visibleAnnotations, "tech.konata.obfuscation.Indy");
        boolean clazz = hasAnnotation(methodWrapper.getOwner().classNode.visibleAnnotations, "tech.konata.obfuscation.Indy");

        if (!(method || clazz))
            return true;

        String type = getEnumAnnotationValue(method ? methodWrapper.getMethodNode().visibleAnnotations : methodWrapper.getOwner().classNode.visibleAnnotations, "tech.konata.obfuscation.Indy", "strength");
