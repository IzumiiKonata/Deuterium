...
public abstract class AbstractRenderer<T> {
	 ...
	 *   真正填充的数组元素个数
	 */
	public int renderAll(byte[] bs, int offset, int length) {
	...
	/**
	 * 一次性渲染完整个音频, 以 short 数组的方式获取采样数据
	 * @param bs
	 *   获取采样数据的数组, short 格式
	 * @param offset
	 *   bs 存放数据的起始位置
	 ...
	 *   真正填充的数组元素个数
	 */
	public int renderAll(short[] bs, int offset, int length) {
		int bOffset = offset; // bs 的 offset
