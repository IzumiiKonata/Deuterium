package tech.konata.phosphate.rendering.phosphor.mod.world.lighting;

import tech.konata.phosphate.rendering.phosphor.api.IChunkLighting;
import tech.konata.phosphate.rendering.phosphor.api.IChunkLightingData;
import tech.konata.phosphate.rendering.phosphor.api.ILightingEngine;
    ...
    public static void readNeighborLightChecksFromNBT(final Chunk chunk, final NBTTagCompound nbt) {
        if (nbt.hasKey(neighborLightChecksKey, 9)) {
            ...
            if (list.tagCount() == FLAG_COUNT) {
                ...
                Minecraft.getLogger().warn("Chunk field {} had invalid length, ignoring it (chunk coordinates: {} {})", neighborLightChecksKey, chunk.xPosition, chunk.zPosition);
            }
        }
    }
