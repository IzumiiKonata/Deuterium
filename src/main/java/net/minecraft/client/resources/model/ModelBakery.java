...
public class ModelBakery {
    private static final Set<Location> LOCATIONS_BUILTIN_TEXTURES = Sets.newHashSet(Location.of("blocks/water_flow"), Location.of("blocks/water_still"), Location.of("blocks/lava_flow"), Location.of("blocks/lava_still"), Location.of("blocks/destroy_stage_0"), Location.of("blocks/destroy_stage_1"), Location.of("blocks/destroy_stage_2"), Location.of("blocks/destroy_stage_3"), Location.of("blocks/destroy_stage_4"), Location.of("blocks/destroy_stage_5"), Location.of("blocks/destroy_stage_6"), Location.of("blocks/destroy_stage_7"), Location.of("blocks/destroy_stage_8"), Location.of("blocks/destroy_stage_9"), Location.of("items/empty_armor_slot_helmet"), Location.of("items/empty_armor_slot_chestplate"), Location.of("items/empty_armor_slot_leggings"), Location.of("items/empty_armor_slot_boots"));
    ...
    private final Map<Location, TextureAtlasSprite> sprites = Maps.newHashMap();
    private final Map<Location, ModelBlock> models = Maps.newLinkedHashMap();
    ...
    private final Map<String, Location> itemLocations = Maps.newLinkedHashMap();
    private final Map<Location, ModelBlockDefinition> blockDefinitions = Maps.newHashMap();
    ...
    private void loadVariantItemModels() {
        ...
        this.variants.put(MODEL_MISSING, new ModelBlockDefinition.Variants(MODEL_MISSING.getVariant(), Lists.newArrayList(new ModelBlockDefinition.Variant(Location.of(MODEL_MISSING.getResourcePath()), ModelRotation.X0_Y0, false, 1))));
        Location resourcelocation = Location.of("item_frame");
    ...
    private ModelBlockDefinition getModelBlockDefinition(Location p_177586_1_) {
        Location resourcelocation = this.getBlockStateLocation(p_177586_1_);
    ...
    private Location getBlockStateLocation(Location p_177584_1_) {
        return Location.of(p_177584_1_.getResourceDomain(), "blockstates/" + p_177584_1_.getResourcePath() + ".json");
    }
    ...
    private void loadVariantModels() {
        for (ModelResourceLocation modelresourcelocation : this.variants.keySet()) {
            for (ModelBlockDefinition.Variant modelblockdefinition$variant : this.variants.get(modelresourcelocation).getVariants()) {
                Location resourcelocation = modelblockdefinition$variant.getModelLocation();
    ...
    private Location getModelLocation(Location p_177580_1_) {
        Location resourcelocation = p_177580_1_;
        String s = p_177580_1_.getResourcePath();

        if (!s.startsWith("mcpatcher") && !s.startsWith("optifine")) {
            return Location.of(p_177580_1_.getResourceDomain(), "models/" + p_177580_1_.getResourcePath() + ".json");
        } else {
            if (!s.endsWith(".json")) {
                resourcelocation = Location.of(p_177580_1_.getResourceDomain(), s + ".json");
            }
    ...
    private void loadItemModels() {
        ...
        for (Item item : Item.itemRegistry) {
            for (String s : this.getVariantNames(item)) {
                Location resourcelocation = this.getItemLocation(s);
    ...
    public void loadItemModel(String p_loadItemModel_1_, Location p_loadItemModel_2_, Location p_loadItemModel_3_) {
    ...
    private Location getItemLocation(String p_177583_1_) {
        Location resourcelocation = Location.of(p_177583_1_);

        return Location.of(resourcelocation.getResourceDomain(), "item/" + resourcelocation.getResourcePath());
    }
    ...
    private void bakeBlockModels() {
        ...
        for (Entry<String, Location> entry : this.itemLocations.entrySet()) {
            Location resourcelocation = entry.getValue();
    ...
    public IBakedModel bakeModel(ModelBlock modelBlockIn, ModelRotation modelRotationIn, boolean uvLocked) {
        TextureAtlasSprite textureatlassprite = this.sprites.get(Location.of(modelBlockIn.resolveTextureName("particle")));
    ...
    private void loadModels() {
        Deque<Location> deque = Queues.newArrayDeque();
        Set<Location> set = Sets.newHashSet();

        for (Location resourcelocation : this.models.keySet()) {
            set.add(resourcelocation);
            Location resourcelocation1 = this.models.get(resourcelocation).getParentLocation();
        ...
        while (!deque.isEmpty()) {
            Location resourcelocation2 = deque.pop();
            ...
            try {
                ...
                Location resourcelocation3 = modelblock.getParentLocation();
    ...
    private List<Location> getParentPath(Location p_177573_1_) {
        List<Location> list = Lists.newArrayList(p_177573_1_);
    ...
    private Location getParentLocation(Location p_177576_1_) {
        for (Entry<Location, ModelBlock> entry : this.models.entrySet()) {
    ...
    private Set<Location> getTextureLocations(ModelBlock p_177585_1_) {
        Set<Location> set = Sets.newHashSet();

        for (BlockPart blockpart : p_177585_1_.getElements()) {
            for (BlockPartFace blockpartface : blockpart.mapFaces.values()) {
                Location resourcelocation = Location.of(p_177585_1_.resolveTextureName(blockpartface.texture));
                set.add(resourcelocation);
            }
        }

        set.add(Location.of(p_177585_1_.resolveTextureName("particle")));
        return set;
    }
    ...
    private void loadSprites() {
        ...
        this.sprites.put(Location.of("missingno"), this.textureMap.getMissingSprite());
    }

    private Set<Location> getItemsTextureLocations() {
        Set<Location> set = Sets.newHashSet();

        for (Location resourcelocation : this.itemLocations.values()) {
            ModelBlock modelblock = this.models.get(resourcelocation);

            if (modelblock != null) {
                set.add(Location.of(modelblock.resolveTextureName("particle")));

                if (this.hasItemModel(modelblock)) {
                    for (String s : ItemModelGenerator.LAYERS) {
                        Location resourcelocation2 = Location.of(modelblock.resolveTextureName(s));
                    ...
                    for (BlockPart blockpart : modelblock.getElements()) {
                        for (BlockPartFace blockpartface : blockpart.mapFaces.values()) {
                            Location resourcelocation1 = Location.of(modelblock.resolveTextureName(blockpartface.texture));
    ...
    public ModelBlock getModelBlock(Location p_getModelBlock_1_) {
    ...
    public static void fixModelLocations(ModelBlock p_fixModelLocations_0_, String p_fixModelLocations_1_) {
        Location resourcelocation = fixModelLocation(p_fixModelLocations_0_.getParentLocation(), p_fixModelLocations_1_);
    ...
    public static Location fixModelLocation(Location p_fixModelLocation_0_, String p_fixModelLocation_1_) {
        if (p_fixModelLocation_0_ != null && p_fixModelLocation_1_ != null) {
            if (p_fixModelLocation_0_.getResourceDomain().equals("minecraft")) {
                ...
                if (s1 != s) {
                    p_fixModelLocation_0_ = Location.of(p_fixModelLocation_0_.getResourceDomain(), s1);
                }
