...
public class SoundHandler implements IResourceManagerReloadListener, ITickable {
    ...
    public static final SoundPoolEntry missing_sound = new SoundPoolEntry(Location.of("meta:missing_sound"), 0.0D, 0.0D, false);
    ...
    public void onResourceManagerReload(IResourceManager resourceManager) {
        ...
        for (String s : resourceManager.getResourceDomains()) {
            try {
                for (IResource iresource : resourceManager.getAllResources(Location.of(s, "sounds.json"))) {
                    try {
                        Map<String, SoundList> map = this.getSoundMap(iresource.getInputStream());

                        for (Entry<String, SoundList> entry : map.entrySet()) {
                            this.loadSoundResource(Location.of(s, entry.getKey()), entry.getValue());
                        }
    ...
    private void loadSoundResource(Location location, SoundList sounds) {
        ...
        for (final SoundList.SoundEntry soundlist$soundentry : sounds.getSoundList()) {
            ...
            switch (soundlist$soundentry.getSoundEntryType()) {
                case FILE:
                    Location resourcelocation1 = Location.of(s1, "sounds/" + resourcelocation.getResourcePath() + ".ogg");
                ...
                case SOUND_EVENT:
                    isoundeventaccessor = new ISoundEventAccessor<SoundPoolEntry>() {
                        final Location field_148726_a = Location.of(s1, soundlist$soundentry.getSoundEntryName());
    ...
    public SoundEventAccessorComposite getSound(Location location) {
     ...
     * Play a sound
     */
    public Location playSound(ISound sound) {
        return this.sndManager.playSound(sound);
    }
    ...
    public SoundEventAccessorComposite getRandomSoundFromCategories(SoundCategory... categories) {
        ...
        for (Location resourcelocation : this.sndRegistry.getKeys()) {
