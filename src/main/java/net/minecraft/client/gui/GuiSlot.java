...
public abstract class GuiSlot {
    ...
    public void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
        if (this.isMouseYWithinSlotBounds(this.mouseY)) {
            if (clickedMouseButton == 0 && Mouse.isButtonDown(0) && this.getEnabled()) {
    ...
    public void handleMouseInput(int button, boolean pressed) {
        if (this.isMouseYWithinSlotBounds(this.mouseY)) {
            if (button == 0 && pressed && this.mouseY >= this.top && this.mouseY <= this.bottom) {
            ...
            if (button == 0 && pressed && this.getEnabled()) {
                if (this.initialClickY != -1) {
                    if (this.initialClickY >= 0) {
                        this.amountScrolled -= (float) (this.mouseY - this.initialClickY) * this.scrollMultiplier;
                        this.initialClickY = this.mouseY;
    ...
    public void handleDWheel(int wheel) {
        if (isMouseYWithinSlotBounds(this.mouseY)) {
            ...
            if (i2 != 0) {
                ...
                this.amountScrolled += (float) (i2 * this.slotHeight / 2) * (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) ? 5 : 1);
            }
        }
    }
