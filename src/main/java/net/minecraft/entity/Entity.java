    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            try {
                ...
                CrashReport crashreport = CrashReport.makeCrashReport(throwable, "检查实体方块碰撞");
                CrashReportCategory crashreportcategory = crashreport.makeCategory("正在检查碰撞的实体");
                ...
                for (int j = blockpos.getY(); j <= blockpos1.getY(); ++j) {
                    for (int k = blockpos.getZ(); k <= blockpos1.getZ(); ++k) {
                        ...
                        try {
                            ...
                            CrashReport crashreport = CrashReport.makeCrashReport(throwable, "实体与方块发生碰撞");
                            CrashReportCategory crashreportcategory = crashreport.makeCategory("发生碰撞的方块");
...
//        return /*this.worldObj.isBlockLoaded(blockpos) ? */this.worldObj.getCombinedLight(blockpos, 0)/* : 0*/;
    ...
    public Vec3 getLook(float yaw, float pitch, float prevYaw, float prevPitch, float partialTicks) {
        if (partialTicks == 1.0F) {
    ...
    public double getDistanceToPosition(double x, double y, double z) {
